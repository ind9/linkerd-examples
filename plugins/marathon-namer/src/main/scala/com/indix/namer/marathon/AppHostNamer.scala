...
class AppHostNamer(
  ...
  import AppHostNamer._

  private[this] implicit val _timer = timer

  /**
    * Accepts names in the form:
    *   /<app-host>/residual/path
    *
    *   or
    *
    *   /<app-host>/residual/path
  ...
  def lookup(path: Path): Activity[NameTree[Name]] =
    if (path.isEmpty) Activity.value(NameTree.Neg)
      ...
      appsActivity.map { apps =>
        val allHostsToAppSpec = apps.flatMap(spec => spec.toPaths.map(_ -> spec)).toMap
        Trace.recordBinary("marathon.path", path.show)
        val found = possibleIds.collectFirst {
          case app if allHostsToAppSpec.contains(app) =>
            val appSpec = allHostsToAppSpec(app)
            Trace.recordBinary("marathon.appId", app.show)
            val residual = path.drop(app.size)
            val id = prefix ++ app
            val addr = getAndMonitorAddr(appSpec)
      ...
      def loop(): Unit = if (!stopped) {
        pending = api.getAppIds().respond {
          case Return(apps) =>
            ...
            Trace.recordBinary("marathon.apps", apps.flatMap(_.toPaths).map(_.show).mkString(","))
            if (!stopped) {
  ...
  private[this] var appMonitors: Map[AppSpec, Var[Addr]] = Map.empty

  private[this] def getAndMonitorAddr(app: AppSpec): Var[Addr] = synchronized {

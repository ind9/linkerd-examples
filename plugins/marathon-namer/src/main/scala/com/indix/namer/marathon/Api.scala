...
trait Api {
  ...
  def getAddrs(app: AppSpec): Future[Set[Address]]
}
...
case class AppSpec(
  ...
  def toPaths: Set[Path] = hosts.map(host => Path.read("/" + host))
}
...
object Api {
  ...
  final case class Labels(@JsonProperty("router.hosts") hosts: Option[String]) {
    def allHosts = hosts.map(_.split(":").toSet)
  }
  ...
  private[namer] def rspToAppIds(rsp: http.Response): Future[Api.AppIds] =
    rsp.status match {
      case http.Status.Ok =>
  ...
  private[namer] def rspToAddrs(rsp: http.Response, useHealthCheck: Boolean): Future[Set[Address]] =
    rsp.status match {
      case http.Status.Ok =>
  ...
  private[this] def toAppIds(appsRsp: AppsRsp): Api.AppIds = {
    appsRsp.apps match {
      case Some(apps) =>
        apps.collect {
          case App(Some(id), _, _, Some(labels)) =>
            AppSpec(id.toLowerCase, labels.allHosts.getOrElse(Set()))
        }.toSet
      case None => Set.empty
    }
  }
...
private class AppIdApi(client: Api.Client, apiPrefix: String, useHealthCheck: Boolean)
  ...
  def getAddrs(app: AppSpec): Future[Set[Address]] = {
    val req = http.Request(s"$apiPrefix/apps${app.id}?embed=app.tasks")
